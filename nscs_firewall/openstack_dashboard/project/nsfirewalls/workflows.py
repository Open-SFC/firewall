...
class CreateConfigInfo(workflows.Step):
    ...
    contributes = ("config_name", "config_mode")
...
class CreateConfiguration(workflows.Workflow):
    ...
    success_url = "horizon:project:nsfirewalls:index"
    default_steps = (CreateConfigInfo,
                    )

    def format_status_message(self, message):
        name = self.context.get('config_name') 
        return message % name
    ...
    def handle(self, request, data):
	    ...
	    is_config = api.crdfwaas.config_handle_list_for_tenant(request, tenant_id, 
                                                 name=data['config_name'])
	    #####
	    if is_config:
		return False
	    else:
		config = api.crdfwaas.config_handle_create(request,
						     name=data['config_name'],
						     status=1,
						     slug='firewall',
